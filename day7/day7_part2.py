...
def classifyHand(hand):
  ...
  try:
    if cardtypes == 1:
      cardcount['A'] = cardcount.pop('J')
      jokersub = 'A'
    else:
      n = cardcount.pop('J')
      x = list(cardcount.items())
      x.sort(key=lambda x: cardScore(x[0]))
...
def parse_input(filename):
  ...
  for line in file.readlines():
    es = line.split()
    handtype, cardcount = classifyHand(es[0])
    hands.append({
      'hand': es[0]
    , 'bid': int(es[1])
    , 'info': (handtype, cardcount)
    })
  return hands
...
if __name__ == '__main__':
  ...
  #hands = sortAllHands(parse_input('test_input.txt'))
  print(*hands, sep='\n')
